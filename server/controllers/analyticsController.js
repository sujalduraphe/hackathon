...
export async function institutionOverview(req, res) {
  ...
  // demand: share of postings asking for each skill
  const demand = {};
  ...
  const skillGaps = Object.entries(demand).map(([skill, n]) => {
    ...
    return {
      skill,
      demand: Math.round((n / jobs.length) * 100),
  ...
  const readiness = students.map(s => {
    const best = closestRole(plain(s.skills), jobs);
  ...
  res.json({ college: u.organization, students: students.length, openings: jobs.length, skillGaps, funnel, readiness });
}

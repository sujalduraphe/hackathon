...
export default function StudentDashboard({ onNavigate }) {
  const { profile, jobs, applications, assessments, market } = useAppState();
  const user = profile;
  ...
  const bestRole = closestRole(profile.skills, jobs, market?.roles);

  const jobById = Object.fromEntries(jobs.map(j => [j.id, j]));
  const activity = [
    ...assessments.map(a => ({ text: `Completed ${a.category} assessment (${a.score}%)`, at: a.at })),
  ...
  const statCards = [
    { label: 'Avg Skill Level', value: `${avgSkill}%`, color: '#111111', change: `${verified} verified skills`, up: true },
    { label: 'Applications', value: String(myApps.length), color: '#10b981', change: `${activeApps} active`, up: true },
    { label: 'Assessments', value: String(assessments.length), color: '#f59e0b', change: 'taken on portal', up: true },
    { label: 'Strong Matches', value: String(strongMatches), color: '#f43f5e', change: `of ${jobs.length} openings`, up: true },
  ];
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 600, marginBottom: 4, color: '#111827' }}>
              {hasSkills
                ? <>Readiness for {bestRole.role}: <span style={{ color: '#111111' }}>{bestRole.readiness}%</span></>
        ...
        <div className="chart-container">
          ...
          {hasSkills && <ResponsiveContainer width="100%" height={260}>
            <RadarChart data={skillRadarData}>
              ...
              <Radar name="Skills" dataKey="value" stroke="#111111" fill="#111111" fillOpacity={0.25} strokeWidth={2} />
              <Tooltip
        ...
        <div className="grid-auto">
          ...
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 2, color: '#111827' }}>{job.title}</div>
                  ...
                  <div className="job-card-meta">
                    ...
                    <span className="job-meta-item"><span></span>{job.duration}</span>
                  </div>
                </div>
              </div>
